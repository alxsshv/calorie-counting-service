...
@NoArgsConstructor
public class User {
    /**Числовой идентификатор запси о пользователе в БД.*/
    @Id
    ...
    @Column(name = "age")
    private String age;
    /**Вес пользователя в киллограммах.*/
    @Column(name = "weight")
    private int weight;
     ...
     * другого класса, а так же если оба объкта односятся к классу User,
     * но имеется отличие в значениях свойств объектов.
     * Возвращает true если оба объекта не равны null, односятся к одному
    ...
    /**Метод описывает алгоритм вычисления hash-кода экземпляра калсса User.
     * Для вычисления хешкода используются все поля экземпляра класса User.
     * @return возврашает целочисленное значение хеш-кода экземпляра класса User
