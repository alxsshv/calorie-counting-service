...
/**Интерфейс описывает CRUD методы для сущности прием пищи {@link FoodIntake}) */
@Repository
public interface FoodIntakeRepository extends JpaRepository<FoodIntake, Long> {
    /**Метод для получения приемов пищи {@link FoodIntake} по дате и идентификатору пользователя.
