 ...
 * с сущностью {@link com.alxsshv.model.DayReport} (Отчет о питании за день).
...
public interface DayReportService {
    ...
    /**Метод для для проверки уложился ли пользователь в дневную норму калорий.
     * @param userId - числовой идентификатор пользователя,
     * для которого необходимо выполнить проверку.
     * @param date - дата на которую необходимо выполнить проверку.
     * @return возвращает true, если пользователь уложился в дневную норму калорий
     * (колиество потребленнных калорий меньше или равно дневной нормы калорий,
